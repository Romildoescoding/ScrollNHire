  ...
  return (
    ...
    <div className="bg-zinc-100 dark:bg-zinc-900 md:py-2 md:pr-2 flex md:max-h-screen min-h-[calc(100svh-56px)] md:min-h-[calc(100dvh-56px)] overflow-hidden">
      ...
      <motion.div
        ...
        className={cn(
          "w-full transition-all duration-500 bg-transparent fixed right-2 h-8 flex z-8",
          uploadProgress > -1 ? "top-[64px]" : "top-0 md:top-[8px]",
        )}
      >
        <div
          className={cn(
      ...
      {/* WHEN IS IS MOBILE VIEW AND GOTTA DISPLAY A FLOATING SIDEBAR */}
      {/* I DON'T WANNA DISPLAY THE SIDEBAR IF USER IS UNAUTHENTICATED AND IS JUST VIEWING A REEL YOU KNOW */}

      {(pathname.startsWith("/reels") ? !!session?.user?.email : true) && (
        <div
          className={cn(
